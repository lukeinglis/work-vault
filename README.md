...
- `/prep-day` -- fetches your calendar, creates meeting note shells, drafts a daily schedule
...
- `/pull-emails` -- pulls labeled emails from Gmail into the vault inbox
- `/pull-slack` -- two-way Slack integration for capturing todos, notes, and commands from your phone
- `/slack-listener` -- continuous Slack polling for mobile command capture
...
- `/sync-sessions` -- summarizes Claude Code sessions and GitHub activity for a given date
- `/sessions` -- shows active Claude Code session tracking
- `/compaction` -- preserves context across long-running sessions
- `/factory-run` -- dispatches factory CEO for autonomous improvement cycles
- `/playbook` -- triggers ACE playbook evolution for agent self-improvement
...
```bash
git clone https://github.com/your-username/work-vault.git
cd work-vault
```
...
1. **Morning:** `/prep-day` -- creates meeting notes, drafts a daily schedule
