...
7. **For each new meeting, gather auto-prep context then create the meeting note:**
   ...
   **7a. Auto-prep (lightweight, always runs):**
   ...
   - Add a reference link to that note (e.g., "Last meeting: [[2026-04-23-team-weekly-sync]]")
   - Check for a known shared agenda doc (e.g., `{{SHARED_DOC_ID}}`). If one exists, fetch it and include any new agenda items
...
10. **Build the Daily Focus summary (~4 bullets max):**
   ...
   **Output:** Flatten all sources into ~4 bullets, prioritizing by urgency. Each bullet names the source type. Example:
   - Overdue: Q3 blog post (due Mon)
   - Email: teammate re: partner engineering follow-up
   - {{REPO_1}}: 2 PRs merged (feature-x, docs update)
   - Fine-Tuning: nothing pressing, {{REPO_2}}/{{REPO_3}} quiet
...
11. **Deep competitive research (Mondays only):**
   ...
   **Gate:** Check if target day is Monday. If not Monday, skip to Step 12 and state: "Step 11: skipped (not Monday)."
   ...
   - This launches four parallel background agents (one per initiative) -- each takes 20-30 minutes
