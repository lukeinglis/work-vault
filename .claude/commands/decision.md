...
Examples:
- `/decision We chose Postgres over DynamoDB because we need complex queries`
- `/decision The API gateway will use reverse proxy because of latency requirements`
...
2. **Determine the scope and location:**
   ...
   - Use these signals to determine:
     - Match keywords to the appropriate `01-Components/<component>/decisions/` folder
     - If the decision clearly belongs to one component, route there
     - If cross-cutting or ambiguous, ask the user which component it belongs to
     - Vault infrastructure decisions go in the vault's own component
...
3. **Generate the filename:**
   ...
   - Example: `2026-04-14-postgres-over-dynamodb.md`
...
4. **Create the decision file:**
   ...
   - Fill in:
     ...
     - `date:` -- today's date
     - `initiative:` -- the component slug (<component-1-slug> | <component-2-slug> | <component-3-slug>)
